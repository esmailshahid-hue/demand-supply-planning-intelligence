...
import { type ForecastResult, methodNames, number, percent, shortDate, signedPercent } from './api';
...
export default function Evaluation({ result }: { result: ForecastResult }) {
  ...
  return <section className="panel">
    <div className="section-heading"><div><p className="eyebrow">Historical error</p><h2>How the methods compare</h2></div><div className="segmented" aria-label="Evaluation period">
    ...
    <div className="evaluation-intro"><p>{phase === 'selection' ? `Rolling origins select the method. Evidence closes before ${shortDate(result.selection_cutoff)}.` : `The final 28 historical days test the selected method. These results do not choose the winner.`}</p>
