...
import { api, type Catalog, type ForecastResult, type Size, methodNames, number, percent, shortDate } from './api';
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar"><a className="brand" href="#" onClick={e => { e.preventDefault(); setScreen('Demand Review'); }}><span className="brand-mark" aria-hidden="true">▥</span><span>Demand & Supply<small>PLANNING INTELLIGENCE</small></span></a>
      ...
      <nav aria-label="Primary navigation">{screens.map((name, i) => <button key={name} className={screen === name ? 'nav-item active' : 'nav-item'} aria-current={screen === name ? 'page' : undefined} onClick={() => setScreen(name)}><span className="nav-number">0{i + 1}</span>{name}{(i === 0 || i === 2) && <span className="nav-lock" aria-label="Not yet available">○</span>}</button>)}</nav>
    ...
    <div className="workspace"><header className="topbar"><span>Planning workspace <span className="crumb">/ {screen}</span></span><span className="sample-label">SYNTHETIC DATA <span>· SAR · Riyadh</span></span></header>
      ...
      </> : <section className="panel unavailable"><span className="unavailable-icon" aria-hidden="true">{screen === 'Plan Review' ? '↔' : '⌘'}</span><p className="eyebrow">Not available in Pass 1</p><h2>{screen === 'Plan Review' ? 'Purchasing and allocation come next.' : 'Scenario comparisons need a validated plan.'}</h2><p>{screen === 'Plan Review' ? 'Pass 2 adds shared-stock allocation, purchasing, cash schedules and independent feasibility checks. There are no plan recommendations yet.' : 'Pass 3 adds demand, supplier and funding changes, with frozen-plan and replanned comparisons.'}</p><button className="primary-button" onClick={() => setScreen('Demand Review')}>Review the live forecast →</button></section>}
