...
export default function App() {
  ...
  const reopen=async(review:components['schemas']['ReviewView'],signal:AbortSignal)=>{
    ...
    setSku(nextCatalog.products[0].sku);setLocation(nextCatalog.locations[0].location_id);setOpenedPlan(p);setPlanContext(null);setLinkedForecast(null);setScreen('Plan Review');
  };
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar"><a className="brand" href="#" onClick={e => { e.preventDefault(); setScreen('Demand Review'); }}><span className="brand-mark" aria-hidden="true">▥</span><span>Demand & Supply<small>PLANNING INTELLIGENCE</small></span></a>
      ...
      <nav aria-label="Primary navigation">{screens.map((name, i) => <button key={name} className={screen === name ? 'nav-item active' : 'nav-item'} aria-current={screen === name ? 'page' : undefined} onClick={() => { if(name === 'Scenarios') setLinkedForecast(null); setScreen(name); }}><span className="nav-number" aria-hidden="true">0{i + 1}</span>{name}</button>)}</nav>
      <div className="sidebar-note"><span className="live-dot"/> {source?.provenance?.source==='portable'?'Portable snapshot':source?'Uploaded dataset':'Synthetic portfolio'}<p>Transparent planning evidence.</p><small>{source?'Private temporary server processing.':'Independent example. No company-supplied data.'}</small></div>
    </aside>
