...
import { api, type Catalog, type ForecastResult, type Size, displayedMaeImprovement, fixedNumber, methodNames, number, percent, shortDate, signedPercent } from './api';
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <div className="workspace"><header className="topbar"><span>Planning workspace <span className="crumb">/ {screen}</span></span><span className="sample-label">SYNTHETIC DATA <span>· SAR · Riyadh</span></span></header>
      ...
      </> : screen === 'Plan Review' ? (busy ? <div className="loading">Finishing the current forecast…</div> : <PlanReview/>) : <section className="panel unavailable"><span className="unavailable-icon" aria-hidden="true"><ScenarioIcon/></span><p className="eyebrow">Available in Pass 3</p><h2>Scenario comparisons need a validated plan.</h2><p>Pass 3 adds demand, supplier and funding changes, with frozen-plan and replanned comparisons.</p><button className="primary-button" onClick={() => setScreen('Demand Review')}>Review the live forecast →</button></section>}
