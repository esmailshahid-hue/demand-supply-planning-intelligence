...
export default function App() {
  ...
  const [workflowMutation,setWorkflowMutation]=useState(false);
  const clearWorkflow=()=>{setOpenedPlan(null);setPlanContext(null);setScenarioInitial(null);setScenarioDerived(null);setReviewRecovery(null);setPlanGate(null);setWorkflowMutation(false);setLinkedForecast(null);};
  const useData=(data:components['schemas']['ImportResult'])=>{setBusy(true);setDatasetReference(data.reference!.object_id);setSource(data);clearWorkflow();setSku(data.catalog!.products[0].sku);setLocation(data.catalog!.locations[0].location_id);setScreen('Plan Review');};
  const resetData=()=>{setDatasetReference(null);setSource(null);clearWorkflow();setSku('SKU001');setLocation('S1');setScreen('Demand Review');setRefresh(v=>v+1);};
  ...
  const changeSize = (value: Size) => { setSize(value); setSku('SKU001'); setCatalog(null); clearWorkflow(); };
  const currentPlanBlocked=!!planContext&&(!planGate||planGate.key!==planKey(planContext.plan)||planGate.gate.blocked);
  const scenariosDisabled=workflowMutation||!!scenarioDerived||currentPlanBlocked;
  const openScenarios=(context:PlanContext|null=planContext)=>{if(workflowMutation||scenarioDerived||(context&&(!planGate||planGate.key!==planKey(context.plan)||planGate.gate.blocked)))return;setScenarioInitial(context);setLinkedForecast(null);setScreen('Scenarios');};
  const returnToOriginalScenario=()=>{const original=scenarioDerived?.original||null;setScenarioDerived(null);setReviewRecovery(null);setOpenedPlan(original?.plan||null);setPlanContext(original);setScenarioInitial(original);setPlanGate(original?{key:planKey(original.plan),gate:{blocked:false,reason:''}}:null);setScreen('Scenarios');};
  return <div className="app-shell">
    <a className="skip-link" href="#main-content">Skip to content</a>
    <aside className="sidebar"><a className="brand" href="#" aria-disabled={workflowMutation||undefined} onClick={e => { e.preventDefault(); if(!workflowMutation)setScreen('Demand Review'); }}><span className="brand-mark" aria-hidden="true">▥</span><span>Demand & Supply<small>PLANNING INTELLIGENCE</small></span></a>
      <div className="workspace-label">RIYADH RETAIL NETWORK</div>
      <nav aria-label="Primary navigation">{screens.map((name, i) => {const disabled=workflowMutation||(name==='Scenarios'&&scenariosDisabled);return <button key={name} className={screen === name ? 'nav-item active' : 'nav-item'} aria-current={screen === name ? 'page' : undefined} disabled={disabled} title={name==='Scenarios'&&scenariosDisabled?(scenarioDerived?'Return to the original scenario baseline before starting another comparison.':planGate?.gate.reason||'Finish the current review before testing scenarios.'):undefined} onClick={() => name==='Scenarios'?openScenarios():setScreen(name)}><span className="nav-number" aria-hidden="true">0{i + 1}</span>{name}</button>;})}</nav>
      <div className="sidebar-note"><span className="live-dot"/> {workflowMutation?'Saving review mutation':source?.provenance?.source==='portable'?'Portable snapshot':source?'Uploaded dataset':'Synthetic portfolio'}<p>Transparent planning evidence.</p><small>{workflowMutation?'Navigation resumes when the mutation response is safely recorded.':source?'Private temporary server processing.':'Independent example. No company-supplied data.'}</small></div>
    </aside>
    <div className="workspace"><header className="topbar"><span>Planning workspace <span className="crumb">/ {screen}</span></span><span className="sample-label">{source?.provenance?.source==='portable'?'PORTABLE SNAPSHOT':source?'UPLOADED DATA':'SYNTHETIC DATA'} <span>· SAR · Riyadh</span></span></header>
      ...
      {screen === 'Data and Assumptions' ? <DataWorkspace onUse={useData} onReset={resetData} onReopen={reopen} reconciliationReference={reviewRecovery?.review.reference||openedPlan?.review_id}/> : screen === 'Demand Review' && linkedForecast ? <><section className="panel"><h2>Selected plan forecast · {linkedForecast.policy}</h2><details><summary>Selected policy and forecast version</summary><p>Scenario assumptions {linkedForecast.assumptions_hash}</p><p>Forecast version {linkedForecast.forecast_version}</p></details><p>Scenario policy buffer: {number(linkedForecast.policy_buffer.units)} units · {linkedForecast.policy_buffer.protection_days} days.</p><p>Historical evaluation stays unchanged. Scenario adjustments below apply only to future expected demand.</p>{linkedForecast.adjustments.map(a=><p key={a.day}>{a.day}: {number(a.original)} → {number(a.adjusted)} units · {a.reason}</p>)}<button onClick={()=>setLinkedForecast(null)}>Return to sample forecast controls</button></section><Demand result={linkedForecast.forecast}/></> : screen === 'Demand Review' ? <>
        ...
        <section className="filters" aria-label="Sample controls"><label>Dataset<select value={source?'uploaded':size} onChange={e => changeSize(e.target.value as Size)} disabled={busy||!!source}>{source?<option value="uploaded">{source?.provenance?.source==='portable'?'Portable snapshot':'Uploaded dataset'}</option>:<><option value="fixture">Small fixture · 10 SKUs</option><option value="full">Full sample · 60 SKUs</option></>}</select></label>
      ...
      </> : screen === 'Plan Review' ? (busy ? <div className="loading" role="status">Finishing the current forecast…</div> : <PlanReview key={source?.reference?.object_id||'sample'} initialPlan={openedPlan} recovery={reviewRecovery} scenarioDerived={!!scenarioDerived&&scenarioDerived.reviewId===(openedPlan?.review_id||planContext?.plan.review_id)} mutationPending={workflowMutation} onReviewed={reviewPlan} uploaded={!!source} onReady={rememberPlan} onForecast={openForecast} onScenarios={(plan,nextSize)=>openScenarios({plan,size:nextSize})} onReturnToOriginalScenario={returnToOriginalScenario} onScenarioState={updateScenarioState} onRecovery={recordRecovery} onRecoveryClear={clearRecovery} onMutation={setWorkflowMutation}/>) : <Scenarios uploaded={!!source} firstSku={catalog?.products[0]?.sku} firstStore={catalog?.locations[0]?.location_id} initial={scenarioInitial} onForecast={openForecast} onReview={p=>{setScenarioDerived({reviewId:p.review_id!,original:scenarioInitial});setReviewRecovery(null);setOpenedPlan(p);setScreen('Plan Review');}}/>}
