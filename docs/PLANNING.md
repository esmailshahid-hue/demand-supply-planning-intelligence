...
`planning/constraints.py` represents exact reviewed purchases/movements and candidate-specific prohibitions. Stable keys describe business terms, not optimizer row positions. The benchmark seeds locks and reserves their dated resources; the joint model applies exact equalities/prohibitions. Missing or infeasible locks are conflicts, never silently dropped during fallback. Final independent replay and constraint comparison determine executability. Pooled dated stock remains the dependency authority; there is no purchase-to-transfer ownership fiction.

`planning/review.py` versions decisions and marks every mutation stale until complete regeneration. Final acceptance recomputes the matching forecast/scenario envelope, checks exact decisions and independently reconstructed stock/service/cash against the displayed ledger, and requires acknowledgement only for residual service/buffer shortfalls. No hard failure can be acknowledged away. Portable accepted versions are immutable and distinct from fresh drafts. Reconciliation remainders/prohibitions survive regeneration and explicit new-draft creation. See [WORKBOOK.md](WORKBOOK.md) for state, exports, external IDs and local-storage boundaries.
...
## Pass 3 live-budget correction — 18 September 2026

GitHub Actions run `35242117761` measured full first/repeat at 13.009 / 12.283 seconds despite valid deterministic fallbacks. Its profile showed that the 240-series joint challenger spent about three seconds without completing its first required objective, after forecast preparation, benchmark construction and other required validation had already consumed roughly 9.7 seconds. The full live route now gives that challenger no sub-budget and reports `joint_model:not_attempted`; the 40-series fixture retains the two-second challenger. The exact MILP remains available and unchanged for small inputs and focused tests. Every completed joint plan still needs all staged objectives at optimal/zero gap and independent replay; incomplete or unattempted work cannot supply actions.

Immutable sample validation is reused only when `sample()` returns the exact data object and its already computed issues. Custom planning requests and scenario-transformed datasets validate their complete current contents. Forecast range/event/status/training lookups are memoized only within one series calculation and keyed by their full date/cutoff inputs; offer/lane indexes live only within one network preparation. These changes remove repeated computation without changing forecasts, constraints, objectives, benchmark actions, replay or money arithmetic. No complete response or plan is cached.

Final local production planning was fixture **2.690 / 2.468 s** and full **2.985 / 2.946 s**. All four results were independently replayed deterministic benchmark fallbacks with exact commitment/payment reconciliation and responses below 4.5 MB. This is macOS evidence. The exact corrected commit still needs a green Ubuntu/Docker workflow before Pass 3 closes.
...
The live joint optimizer is a size-aware best-effort challenger: **2.0 seconds for up to 40 ranged series and 0 seconds for the 240-series sample**. Its allowance includes construction and cold import and is bounded by the existing 30-second request deadline minus two seconds for final replay and serialization. Forecasts, deterministic benchmark and benchmark replay run before the challenger. A zero allowance is disclosed as `joint_model:not_attempted`; it is not a timeout or optimality claim. The exact optimizer, zero relative-gap setting, hard constraints, objectives and small-model tests are unchanged.
