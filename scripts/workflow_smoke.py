...
def run(url):
    ...
    for size in ('fixture','full'):
        ...
        raw,planning_seconds=request('/api/plan/sample',{'size':size},headers=source);planned=json.loads(raw)
        ...
        assert review['state']=='draft',review['failures']
        ...
        print(json.dumps({'size':size,**metrics,'multipart_bytes':len(multipart),'upload_http_s':upload_seconds,'planning_http_s':planning_seconds,'review_http_s':review_seconds,'accept_http_s':accept_seconds,'export_http_s':export_seconds,'export_bytes':len(workbook),**review['measurements'],'replay':True,'financial_reconciliation':True,'external_ids_unique':True,'snapshot_reopened':True}),flush=True)
